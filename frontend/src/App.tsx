...
function DetailDialog({ detail, onClose }: { detail: { title: string; text: string }; onClose: () => void }) {
    ...
    return (
        ...
        >
            <span className="preview-badge">Sample content</span>
            <h2 id="detail-title">{detail.title}</h2>
            <p>{detail.text}</p>
            <button className="primary-button" autoFocus onClick={() => dialog.current?.close()}>
                Close
            </button>
        </dialog>
    );
...
function App() {
    ...
    const documentRow = (doc: (typeof documents)[number]) => (
        <div className="document-row" key={doc.title}>
            <button className="document-open" onClick={() => setDetail({ title: doc.title, text: doc.text })}>
                <span className={`file-icon ${doc.color}`}>
                    <Icon name="file" size={18} />
                </span>
                <span>
                    <strong>{doc.title}</strong>
                    <small>
                        {doc.type} <span>·</span> {doc.date}
                    </small>
                </span>
            </button>
            <button className={`icon-button bookmark ${saved.includes(doc.title) ? "saved" : ""}`} aria-label={`${saved.includes(doc.title) ? "Unsave" : "Save"} ${doc.title}`} aria-pressed={saved.includes(doc.title)} onClick={() => setSaved(saved.includes(doc.title) ? saved.filter((title) => title !== doc.title) : [...saved, doc.title])}>
    ...
    return (
        <div className={`workspace ${collapsed ? "collapsed" : ""}`}>
            ...
            <aside className={`sidebar ${mobileOpen ? "mobile-open" : ""}`}>
                <a className="brand" href="#ask-ai" aria-label="KnowledgeBridge home" onClick={() => go("Ask AI")}>
                    <img className="wordmark" src={wordmark} alt="KnowledgeBridge" />
                    <img className="compact-logo" src="/logo.png" alt="" />
                </a>
                <nav aria-label="Main navigation">
                    {navigation.map((item) => (
                        <button key={item.label} className={`nav-item ${page === item.label ? "active" : ""}`} onClick={() => go(item.label)} aria-current={page === item.label ? "page" : undefined} title={item.label}>
                            <Icon name={item.icon} />
            ...
            <div className="main-shell">
                <header className="topbar">
                    <button
                        ...
                        onClick={() => {
                            if (window.matchMedia("(max-width: 760px)").matches) setMobileOpen(!mobileOpen);
                    ...
                    >
                        ...
                        <input id="global-search" aria-label="Search sample knowledge base" placeholder="Search knowledge base, documents, entities…" value={search} onChange={(event) => setSearch(event.target.value)} />
                        <kbd>⌘ K</kbd>
                    </form>
                    <button className="organization" onClick={() => setDetail({ title: "Acme Corporation", text: "Acme Corporation is the fictional organization used in this interface preview. Organization switching will be available when accounts are connected." })}>
                        <Icon name="home" size={18} />
                        <span>Acme Corporation</span>
                        <span>⌄</span>
                    </button>
                    <button className="profile" onClick={() => setDetail({ title: "Demo workspace", text: "You are viewing the frontend scaffold. Login, user profiles, and role-based access are not connected yet." })}>
                ...
                <div className="content-layout">
                    <main className="main-content">
                        <div className="page-heading">
                            <div>
                                <h1>{page}</h1>
                                <p>{page === "Ask AI" ? "Get instant answers from your company knowledge." : `Explore your company ${page.toLowerCase()}.`}</p>
                            </div>
                            <span className="preview-badge">Demo workspace</span>
                        </div>
                        {page === "Ask AI" ? (
                            <>
                                <form className="question-panel panel" onSubmit={submit}>
                                    <label htmlFor="question">Your question</label>
                                    <div className="question-input">
                                        <textarea id="question" value={question} onChange={(event) => setQuestion(event.target.value)} />
                                        <button className="send-button" aria-label="Submit question" disabled={!question.trim()}>
                                            <Icon name="send" size={18} />
                                        </button>
                                    </div>
                                    <fieldset>
                                        <legend>
                                            Mode <span title="Search returns documents; Synthesis combines them into an answer.">ⓘ</span>
                                        </legend>
                                        <div className="mode-options">
                                            {["Search", "Synthesis"].map((value) => (
                                                <label className={`mode-option ${mode === value ? "selected" : ""}`} key={value}>
                                                    <Icon name={value === "Search" ? "search" : "spark"} size={18} />
                                                    <span>{value}</span>
                                ...
                                <section className="answer-panel panel">
                                        ...
                                        {mode === "Synthesis" ? (
                                            <>
                                                ...
                                                <div className="citations">
                                                    {documents.slice(0, expanded ? 5 : 3).map((doc, index) => (
                                                        <button className="citation" key={doc.title} onClick={() => setDetail({ title: doc.title, text: doc.text })}>
                                                            <span className={`file-icon ${doc.color}`}>
                                                            ...
                                                            <span>
                                                                ...
                                                                <small className="updated">Updated {doc.date}</small>
                                                            </span>
                                    ...
                                    {mode === "Synthesis" && (
                                        <footer className="answer-footer">
                                            ...
                                            <button className="icon-button" aria-label="Helpful" aria-pressed={feedback === "yes"} onClick={() => setFeedback("yes")}>
                                                <Icon name="up" size={17} />
                                            </button>
                                            <button className="icon-button" aria-label="Not helpful" aria-pressed={feedback === "no"} onClick={() => setFeedback("no")}>
                                            ...
                                            <button
                                                ...
                                                onClick={() => {
                                                    void navigator.clipboard
                                                        ...
                                                        .then(() => setNotice("Sample answer copied."))
                            ...
                            <section className="collection panel">
                                <h2>{page === "Knowledge Base" ? "Sample documents" : page === "Dashboard" ? "Workspace overview" : page}</h2>
                                {page === "Knowledge Base" ? (
                                    <>
                                        {filtered.map(documentRow)}
                                        {!filtered.length && <p>No sample documents match “{search}”.</p>}
                                    </>
                                ) : page === "Entities" ? (
                                    entities.map((entity, index) => (
                                        <button className="entity-row" key={entity} onClick={() => setDetail({ title: entity, text: `${entity} is mentioned in the sample purchasing workflow. Entity extraction is not connected yet.` })}>
                                    ...
                                    <>
                                        <p>
                                            {page === "Dashboard"
                                                ? service
                                                : page === "Uploads"
                                                  ? "Document upload and ingestion are not connected yet. The queue at right shows sample statuses."
                                                  : page === "Usage"
                                                    ? "The sidebar shows sample metrics. Live usage tracking and cost estimates are not connected yet."
                                                    : "Account management and role settings will appear here once authentication is connected."}
                                        </p>
                                        <button className="primary-button" onClick={() => go("Ask AI")}>
                                            Explore Ask AI
                                        </button>
                                    </>
                                )}
                    ...
                    <aside className="context-column" aria-label="Supporting knowledge">
                        <section className="panel context-panel">
                            <h2>
                                <Icon name="users" size={18} /> Detected Entities
                            </h2>
                            {entities.map((entity, index) => (
                                <button key={entity} className="entity-row" onClick={() => setDetail({ title: entity, text: `${entity} is part of the sample purchasing workflow. ${index === 3 ? "This cost center tracks software spending." : "This department reviews requests within its area of responsibility."}` })}>
                                    <span className={`entity-icon color-${index}`}>
                            ...
                            <button className="panel-link" onClick={() => go("Entities")}>
                                View all entities <Icon name="chevron" size={16} />
                            </button>
                        </section>
                        <section className="panel context-panel">
                            <h2>
                                <Icon name="file" size={18} /> Related Documents
                            </h2>
                            ...
                            >
                                View all documents <Icon name="chevron" size={16} />
                            </button>
                        </section>
                        <section className="panel context-panel upload-panel">
                            <h2>
                                <Icon name="upload" size={18} /> Upload / Ingestion Status{" "}
                                <button className="text-button" onClick={() => go("Uploads")}>
                                    View all
                                </button>
                            </h2>
                            {[
                                ...
                                <div className="upload-row" key={file}>
